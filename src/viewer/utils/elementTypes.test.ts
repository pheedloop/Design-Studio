...
describe("displayName", () => {
  it("prefers the element's own name", () => {
    expect(displayName({ name: "Hall A", elementType: "meeting_room" }, t)).toBe("Hall A");
  });

  it("falls back to the type name when unnamed", () => {
    expect(displayName({ name: "", elementType: "meeting_room" }, t)).toBe("Meeting Room");
    expect(displayName({ name: "", elementType: "session_area" }, t)).toBe("Session Area");
  });
...
describe("locationLabel", () => {
  it("uses the name when there is one", () => {
    expect(locationLabel({ name: "Acme Corp", type: "exhibitor" }, t)).toBe("Acme Corp");
  });

  it("falls back to the type name for the two location types", () => {
    expect(locationLabel({ name: "", type: "session_area" }, t)).toBe("Session Area");
    expect(locationLabel({ name: "", type: "meeting_room" }, t)).toBe("Meeting Room");
  });
});
