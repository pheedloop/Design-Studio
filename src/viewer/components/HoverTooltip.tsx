    ...
    >
      {item.type === "booth" && exhibitor ? (
        ...
        <>
          ...
          {item.type === "booth" ? (
            reserved ? (
              <div className="mt-hair text-[11px] font-medium text-amber-600">
                {t("viewer.booth.reserved")}
              </div>
            ) : (
              <div className="mt-hair text-[11px] text-text-subtle">
                {t("viewer.booth.unoccupied")}
              </div>
            )
          ) : (
            <div className="mt-hair text-[11px] font-medium text-text-subtle uppercase tracking-wide">
              {t(
                TYPE_NAME[
