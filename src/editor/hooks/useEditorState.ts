...
function backfillLegendEntryIds(data: FloorPlanData): FloorPlanData {
  const needsBackfill = data.legend.entries.some(
    e => !e.id || e.visible === undefined,
  );
...
function scaleGeometry(geo: Geometry, sx: number, sy: number): Geometry {
  ...
  if ("points" in geo && Array.isArray(geo.points)) {
    g.points = geo.points.map((v: number, i: number) =>
      i % 2 === 0 ? v * sx : v * sy,
    );
  }
...
export function useEditorState(
  ...
  const loadedData = backfillViewerAppearance(
    backfillLegendEntryIds(
      backfillTypeStyles(
        backfillLayers(
          persist ? (loadFromStorage(persistKey) ?? initialData) : initialData,
        ),
      ),
    ),
  );
  const {
    ...
    canUndo,
    canRedo,
  } = useHistory<FloorPlanData>(loadedData);
    ...
    (memberIds: string[], name?: string) => {
      ...
      setData(prev => {
        ...
        return {
          ...
          groups: [
            ...(prev.groups ?? []),
            { id: groupId, name: name ?? "Group" } satisfies GroupDefinition,
          ],
          elements: prev.elements.map(el =>
            idSet.has(el.id)
  ...
  const updateElementType = useCallback(
    (
      id: string,
      newType: ElementType,
      propertyOverrides?: Partial<ElementProperties>,
    ) => {
      setData(prev => ({
        ...prev,
        elements: prev.elements.map(el =>
          el.id === id
            ? {
                ...
                properties: propertyOverrides
                  ? { ...el.properties, ...propertyOverrides }
                  : el.properties,
              }
            : el,
        ),
  ...
  const toggleBackgroundDxfLayer = useCallback(
    (layer: string) => {
      setData(prev => {
    ...
    (rect: { x: number; y: number; width: number; height: number }) => {
      ...
      setData(prev => {
        ...
        if (background?.kind === "image") {
          ...
          const offset = (p: DxfPrimitive): DxfPrimitive =>
            offsetPrimitive(p, -cx, -cy);
          ...
          const inside = (p: DxfPrimitive): boolean => {
            ...
            return (
              b.maxX >= 0 && b.minX <= width && b.maxY >= 0 && b.minY <= height
            );
          };
          ...
          background = {
            ...
            primitives,
            layers,
            hiddenLayers: background.hiddenLayers?.filter(l =>
              layers.includes(l),
            ),
          };
        }
        ...
        const scaleCalibration = prev.scaleCalibration
          ? {
              ...
              p1: {
                x: prev.scaleCalibration.p1.x - cx,
                y: prev.scaleCalibration.p1.y - cy,
              },
              p2: {
                x: prev.scaleCalibration.p2.x - cx,
                y: prev.scaleCalibration.p2.y - cy,
              },
            }
          : undefined;
      ...
      setData(prev => {
        ...
        if (mode === "scale") {
          ...
          if (background?.kind === "image") {
            ...
            background = {
              ...background,
              primitives: background.primitives.map(p =>
                scalePrimitive(p, sx, sy),
              ),
            };
          }
          ...
          const scaleCalibration = prev.scaleCalibration
            ? {
                ...prev.scaleCalibration,
                p1: {
                  x: prev.scaleCalibration.p1.x * sx,
                  y: prev.scaleCalibration.p1.y * sy,
                },
                p2: {
                  x: prev.scaleCalibration.p2.x * sx,
                  y: prev.scaleCalibration.p2.y * sy,
                },
              }
            : undefined;
          ...
          return {
            ...
            dimensions: {
              ...
              width,
              height,
              pixelsPerUnit: prev.dimensions.pixelsPerUnit * sx,
            },
          };
        }
        ...
        const elements = moved
          ? prev.elements.map(el => ({
              ...el,
              geometry: offsetGeometry(el.geometry, dx, dy),
            }))
          : prev.elements;
        ...
        const scaleCalibration =
          moved && prev.scaleCalibration
            ? {
                ...prev.scaleCalibration,
                p1: {
                  x: prev.scaleCalibration.p1.x + dx,
                  y: prev.scaleCalibration.p1.y + dy,
                },
                p2: {
                  x: prev.scaleCalibration.p2.x + dx,
                  y: prev.scaleCalibration.p2.y + dy,
                },
              }
            : prev.scaleCalibration;
    ...
    (id: string, direction: "forward" | "backward" | "front" | "back") => {
      setData(prev => {
        ...
        return {
          ...
          elements: prev.elements.map(el =>
            el.id === id
              ? { ...el, properties: { ...el.properties, zIndex: newZ } }
              : el,
          ),
        };
      });
  ...
  const initWalkableGrid = useCallback(() => {
    setData(prev => {
      ...
      return {
        ...prev,
        walkableLayer: createWalkableGrid(
  ...
  const setWalkableCell = useCallback(
    (col: number, row: number, value: 0 | 1) => {
      setData(prev => {
        const grid = prev.walkableLayer;
        if (!grid || row < 0 || row >= grid.rows || col < 0 || col >= grid.cols)
          return prev;
  ...
  const setWalkableGridResolution = useCallback(
    (cellSize: number) => {
      setData(prev => ({
        ...prev,
        walkableLayer: createWalkableGrid(
  ...
  const setWalkableGrid = useCallback(
    (grid: WalkableGrid) => {
  ...
  const updateViewerAppearance = useCallback(
    (updates: Partial<ViewerAppearance>) => {
      replaceData(prev => ({
        ...prev,
        viewerAppearance: {
          ...DEFAULT_VIEWER_APPEARANCE,
          ...prev.viewerAppearance,
          ...updates,
        },
      }));
    },
