...
The `@/` alias must be wired everywhere source gets compiled, or it breaks silently for whichever build didn't get it:

- `vite.config.ts` (demo app dev/build) — `resolve.alias`
- `tsconfig.app.json` — `compilerOptions.paths` (so the editor and `tsc` agree with Vite)
- `vite.lib.config.ts` (the actual published library build — `editor/`, `viewer/`, `seatviewer/`) — `resolve.alias`
- `tsconfig.lib.json` — `compilerOptions.paths`

The ticket that introduced this rule only named `vite.config.ts` + `tsconfig.app.json`, but the alias has to resolve in the **library** build too — `editor/`, `viewer/`, and `seatviewer/` are exactly the folders that get published, so if any file inside them ever imports with `@/`, `vite.lib.config.ts` needs the same alias or the lib build breaks.
